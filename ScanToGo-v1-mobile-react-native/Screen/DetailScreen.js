...
const ItemText = ({ type, detail }) => {
    return (
        <Text numberOfLines={1} ellipsizeMode="tail" style={{ fontSize: 17, marginTop: 10 }}>{type} - {detail}</Text>
    )
}
    ...
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity style={styles.detail} onPress={() => {
                Linking.openURL("googlechrome://navigate?url=" + detailData.preview_link)
                // navigation.navigate('excelScreenStack', { file: detailData.preview_link })
            }}>
                <Text style={styles.detailInfo}>View File</Text>
                <FontAwesome name="file" size={23} color="#0049EE" style={styles.detailIcon} />
            </TouchableOpacity>
        </View>
    )
}
