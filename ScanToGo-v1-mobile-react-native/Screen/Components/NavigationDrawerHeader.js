...
const NavigationDrawerHeader = (props) => {
  ...
  const [searchString, setSearchString, updateSearchString] = useGlobalState("searchString")
  ...
  const getFileData = async () => {
    setLoading(true)
    await updateIndexPage(() => { return 0 })
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchKeyContainer}>
        {
          searchFlag
            ? <TextInput style={styles.searchKey} placeholder='Search file' placeholderTextColor="#39393a" color={"white"} defaultValue={searchString}
              onChangeText={
                (txt) => {
                  updateSearchString(() => { return txt })
                }}
            />
            : null
        }
